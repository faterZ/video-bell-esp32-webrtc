...
void amplify_audio(uint8_t *buffer, uint32_t framesize, float gain) {
...
uint8_t wav_play_song(uint8_t *fname)
{
    ...
    if (g_audiodev.file || g_audiodev.tbuf)
    {
        ...
        if (res == 0)                               /* 解码成功 */
        {
            ...
            {
                i2s_set_samplerate_bits_sample(wavctrl.samplerate,16);
            }
            else if (wavctrl.bps == 24)
            {
                i2s_set_samplerate_bits_sample(wavctrl.samplerate,24);
            }
